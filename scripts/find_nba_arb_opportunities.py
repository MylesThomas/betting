...
HOW IT WORKS (4 steps):
    ...
    2. For each game, fetch all prop markets
...
OUTPUT FILES:
    - data/04_output/nba/arbs/YYYY-MM-DD/arb_output_YYYYMMDD_HHMMSS.csv (CSV snapshot)
    - s3://betting-nba-arbs/cache/nba_arbs_cache.parquet (persistent cache, updated immediately)
...
CACHE WORKFLOW (Real-time updates):
    ...
    - Parquet cache = single source of truth (3MB vs 100+ individual CSVs)
    - Load time: 1-2 seconds vs 58 seconds
...
def append_to_parquet_cache(new_arbs_df: pd.DataFrame, sport: str = 'nba'):
    """
...
def main(markets=DEFAULT_MARKETS, limit=None, historical_date=None, historical_time="17:00:00", 
    ...
    try:
        ...
        if not is_historical:  # Only update cache for live runs (not historical backfills)
            try:
                append_to_parquet_cache(best_odds_df, sport='nba')
