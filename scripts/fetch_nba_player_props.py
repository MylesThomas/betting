...
# B. Build NBA calendar (used for game dates)
python scripts/nba_calendar_builder.py --season 2025-26 --s3

# C. Set API key in .env
...
Season Dates (config/season_dates.yaml):
    ...
    - Used to filter game dates from NBA calendar

NBA Calendar (data/01_input/nba_calendar/all_games_{season}.csv):
    - Generated by nba_calendar_builder.py
    - Contains all game dates for the season
    - Used to determine which dates to fetch props for
...
Created: 2025-01-05
Updated: 2026-02-08 (added config/calendar integration)
Author: Thomas Myles
"""
...
def load_game_dates_from_calendar(season):
    """
    Load game dates from NBA calendar CSV.
    
    Args:
        season: Season string (e.g., '2025-26')
    
    Returns:
        List of date objects for games in the season
    """
    games_csv = NBA_CALENDAR_PATH / f'all_games_{season.replace("-", "_")}.csv'
    
    if not games_csv.exists():
        logging.error(f"NBA calendar not found: {games_csv}")
        logging.error("Run: python scripts/nba_calendar_builder.py --season {season}")
        return []
    ...
    logging.info(f"📅 Loaded {len(game_dates)} game dates from NBA calendar")
...
def fetch_full_season(upload_s3=True, fetch_games=False):
    ...
    # Load game dates from NBA calendar
    game_dates = load_game_dates_from_calendar(SEASON)
    
    if not game_dates:
        logging.error("Failed to load game dates from NBA calendar")
        return None
    
    # Get season date range from config for validation
    season_start, season_end = get_season_date_range(SEASON)
    
    if season_start and season_end:
        # Filter game dates to season date range
        game_dates = [d for d in game_dates if season_start <= d <= season_end]
        logging.info(f"Season has {len(game_dates)} game dates (filtered to config date range)")
    else:
        logging.info(f"Season has {len(game_dates)} game dates (no config filtering)")
