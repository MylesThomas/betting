...
def load_plays_from_s3(date_str, strategy='both'):
    """
...
def load_results_from_s3(date_str, strategy='both'):
    """
    ...
    for strat in strategies_to_load:
        key = f"{S3_PREFIX_RESULTS}/{strat}/{date_str}.csv"
...
def generate_email_text(df_results, results_date, df_plays, plays_date):
...
def generate_email_html(df_results, results_date, df_plays, plays_date):
    """Generate complete email body in HTML format"""
    # TODO: Implement HTML formatting if needed
    # For now, just wrap text in <pre> tags
    subject, text_body = generate_email_text(df_results, results_date, df_plays, plays_date)
...
def main():
    ...
    # Load data
    df_plays = load_plays_from_s3(plays_date, strategy=args.strategy)
    df_results = load_results_from_s3(results_date, strategy=args.strategy)
    ...
    # Generate email
    if args.format == 'html':
        subject, body = generate_email_html(df_results, results_date, df_plays, plays_date)
    else:
        subject, body = generate_email_text(df_results, results_date, df_plays, plays_date)
