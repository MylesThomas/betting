...
def build_sidebar() -> SidebarFilters:
    with st.sidebar:
    ...
    return SidebarFilters(selected_buckets=selected_buckets)
...
def main() -> None:
    ...
    filters: SidebarFilters = build_sidebar()

    todays_scored_filtered: pd.DataFrame | None = None
    if todays_scored is not None:
        todays_scored_filtered = derive_strategy_bucket(todays_scored)

    date_filtered: pd.DataFrame = apply_date_filter(
        all_settled_raw, date(2026, 4, 7), date.today()
    )
    ...
    with tab3:
        ...
        selected_buckets: list[str] = st.multiselect(
            ...
            options=BT_BUCKETS,
            default=BT_BUCKETS,
            help="both = OLS and XGB both fired · ols / xgb = only that model fired",
            key="bt_bucket_select",
        )
        ...
        if bt_raw.empty:
        ...
        elif not selected_buckets:
            st.info("Select at least one bucket.")
        else:
            bt_filtered = filter_by_buckets(bt_raw, selected_buckets)
            ...
            if bt_settled.empty:
                ...
                st.plotly_chart(build_backtest_pnl_chart(bt_settled, selected_buckets), use_container_width=True)
