"""
Analyze vig in NCAAB Championship (March Madness) futures markets.

Context:
Created to extend the NFL/NBA/NCAAF futures workflow to NCAAB (college basketball).
Part of weekly futures analysis for multiple sports.

Purpose:
- Read most recent NCAAB championship futures
...
Usage:
    ...
    python3 analysis/analyze_ncaab_championship_futures_vig.py
"""
...
def get_most_recent_futures_file():
    """Get the most recent NCAAB Championship futures CSV file"""
    futures_dir = repo_root / 'data/01_input/the-odds-api/ncaab/futures'
    
    # Get all CSV files
    csv_files = sorted(futures_dir.glob('ncaab_championship_futures_*.csv'))
...
def main():
    """Analyze NCAAB championship futures vig"""
    
    print("="*80)
    print("NCAAB CHAMPIONSHIP FUTURES VIG ANALYSIS")
    print("="*80 + "\n")
    ...
    # Save to CSV
    os.makedirs(repo_root / 'data/04_output/ncaab', exist_ok=True)
    output_file = repo_root / 'data/04_output/ncaab/ncaab_championship_fair_odds.csv'
