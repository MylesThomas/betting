"""
Analyze vig in NCAAF Championship (College Football Playoff) futures markets.

Context:
Created to extend the NFL/NBA futures workflow to NCAAF. Part of weekly futures analysis
for multiple sports (NFL, NBA, NCAAF, NCAAB).

Purpose:
- Read most recent NCAAF championship futures
...
Usage:
    ...
    python3 analysis/analyze_ncaaf_championship_futures_vig.py
"""
...
def get_most_recent_futures_file():
    """Get the most recent NCAAF Championship futures CSV file"""
    futures_dir = repo_root / 'data/01_input/the-odds-api/ncaaf/futures'
    
    # Get all CSV files
    csv_files = sorted(futures_dir.glob('ncaaf_championship_futures_*.csv'))
...
def main():
    """Analyze NCAAF championship futures vig"""
    
    print("="*80)
    print("NCAAF CHAMPIONSHIP FUTURES VIG ANALYSIS")
    print("="*80 + "\n")
    ...
    print("-"*80)
    for i, row in fair_df.head(20).iterrows():
        odds_str = f"{row['odds']:+.0f}" if row['odds'] > 0 else f"{row['odds']:.0f}"
        print(f"{fair_df.index.get_loc(i)+1:<6} {row['team']:<40} {odds_str:<10} "
    ...
    print(f"{'Rank':<6} {'Team':<40} {'Record':<10} {'Best Book':<12} {'Best Odds':<12} {'Implied %':<12} {'Fair Odds':<12} {'Fair %':<10}")
    print("-"*130)
    ...
    print("-"*130)
    print(f"{'TOTAL':<6} {'':<40} {'':<10} {'':<12} {'':<12} {team_avg['implied_prob_avg'].sum()*100:>5.2f}%      {'':<12} 100.00%")
    ...
    # Save to CSV
    os.makedirs(repo_root / 'data/04_output/ncaaf', exist_ok=True)
    output_file = repo_root / 'data/04_output/ncaaf/ncaaf_championship_fair_odds.csv'
