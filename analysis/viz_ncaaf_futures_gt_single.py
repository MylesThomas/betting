"""
Create a FiveThirtyEight-style visualization of NCAAF Championship futures using R's gt package.

Context:
Created for weekly college football playoff futures analysis. Part of multi-sport futures workflow
(NFL, NBA, NCAAF, NCAAB). Single table visualization using R's gt package via rpy2.

Note:
Team logos will be sourced from ESPN or other CDN once we identify the proper URLs.
For now, logos will be placeholder or we'll need to implement logo fetching separately.
...
Purpose:
- Read the fair odds CSV from analyze_ncaaf_championship_futures_vig.py
...
Usage:
    ...
    python3 analysis/viz_ncaaf_futures_gt_single.py
    
    # Show only top 10 teams
    python3 analysis/viz_ncaaf_futures_gt_single.py --top-n 10
"""
...
3. College Football Playoff futures — only teams with championship odds shown."""
...
# -----------------------------------------------------------------------------
OUTPUT_FILENAME = "ncaaf_futures_vig_single.png"
...
def get_team_logos(team_names):
    """
    Get NCAAF team logos using ncaa_team_utils.
    
    Uses intelligent mapping that handles edge cases like:
    - "Indiana Hoosiers" → "Indiana.png"
    - "Miami Hurricanes" → "Miami FL.png"
    - "Ole Miss Rebels" → "Mississippi.png"
    ...
    # Use the utility function to map team names to logos
...
def prepare_data_for_visualization(df, logo_map):
    ...
    Args:
        df: Raw dataframe from ncaaf_championship_fair_odds.csv
    ...
    # Add team logo URLs (empty for now)
    ...
    # Count how many teams have logos
    teams_with_logos = df_display['logo_url'].notna().sum()
    
    print(f"   ✅ Prepared {len(df_display)} teams")
    print(f"   ✅ {teams_with_logos}/{len(df_display)} teams have logos")
...
def create_gt_table_with_r(df_display, average_vig_pct, total_teams, top_n):
    ...
    # Select columns for display (with logo if available)
    table_df = df_display[[
    ...
    ro.globalenv['ncaaf_data'] = r_df
    
    # Output path
    output_path = repo_root / 'content/viz/ncaaf' / OUTPUT_FILENAME
    ...
    table <- ncaaf_data %>%
      ...
      # Add team logos using gtExtras (if logo_url is not NA)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create NCAAF championship futures visualization')
    parser.add_argument('--top-n', type=int, default=9999,
                        help='Number of top teams to show (default: 9999 = all teams)')
    args = parser.parse_args()
    
    print("="*80)
    print("NCAAF CHAMPIONSHIP FUTURES VISUALIZATION (R + GT PACKAGE)")
    print("="*80 + "\n")
    ...
    # Read the CSV
    csv_file = repo_root / 'data/04_output/ncaaf/ncaaf_championship_fair_odds.csv'
    metadata_file = repo_root / 'data/04_output/ncaaf/ncaaf_championship_metadata.csv'
    
    if not csv_file.exists():
        print(f"❌ CSV file not found: {csv_file}")
        print("Run analyze_ncaaf_championship_futures_vig.py first!")
        return
    ...
    # Get team logos using smart mapping
    from ncaa_team_utils import get_logo_coverage_stats
    
    team_names = df['team'].tolist()
    logo_map = get_team_logos(team_names)
    
    # Get coverage stats
    stats = get_logo_coverage_stats(team_names, repo_root)
    
    print(f"   🏈 Logo coverage: {stats['matched']}/{stats['total']} teams ({stats['coverage_pct']:.1f}%)")
