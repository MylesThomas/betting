"""
Create a FiveThirtyEight-style visualization of NCAAB Championship futures using R's gt package.

Context:
Created for weekly March Madness futures analysis. Part of multi-sport futures workflow
...
Purpose:
- Read the fair odds CSV from analyze_ncaab_championship_futures_vig.py
...
Usage:
    ...
    python3 analysis/viz_ncaab_futures_gt_single.py
    
    # Show only top 20 teams
    python3 analysis/viz_ncaab_futures_gt_single.py --top-n 20
"""
...
# -----------------------------------------------------------------------------
TITLE = "NCAAB Championship Futures: True Odds vs. What Books Charge"
...
3. March Madness futures — only teams with championship odds shown."""
...
# -----------------------------------------------------------------------------
OUTPUT_FILENAME = "ncaab_futures_vig_single.png"
...
def get_team_logos(team_names):
    """
    Get NCAAB team logos using ncaa_team_utils.
    
    Uses intelligent mapping that handles edge cases like:
    - "UConn Huskies" → "Connecticut.png"
    - "Duke Blue Devils" → "Duke.png"
    - "North Carolina Tar Heels" → "North Carolina.png"
...
def prepare_data_for_visualization(df, logo_map):
    ...
    Args:
        df: Raw dataframe from ncaab_championship_fair_odds.csv
...
def create_gt_table_with_r(df_display, average_vig_pct, total_teams, top_n):
    ...
    ro.globalenv['ncaab_data'] = r_df
    
    # Output path
    output_path = repo_root / 'content/viz/ncaab' / OUTPUT_FILENAME
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Create NCAAB championship futures visualization')
    parser.add_argument('--top-n', type=int, default=9999,
                        help='Number of top teams to show (default: 9999 = all teams)')
    args = parser.parse_args()
    
    print("="*80)
    print("NCAAB CHAMPIONSHIP FUTURES VISUALIZATION (R + GT PACKAGE)")
    print("="*80 + "\n")
    ...
    # Read the CSV
    csv_file = repo_root / 'data/04_output/ncaab/ncaab_championship_fair_odds.csv'
    metadata_file = repo_root / 'data/04_output/ncaab/ncaab_championship_metadata.csv'
    
    if not csv_file.exists():
        print(f"❌ CSV file not found: {csv_file}")
        print("Run analyze_ncaab_championship_futures_vig.py first!")
        return
