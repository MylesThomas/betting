...
Lives under lambda/ncaab_fade_revenge_daily/tmp/ for deployment context.
Run from repo root so PROJECT_ROOT and src/tmp imports resolve.

Usage:
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --conference "Big Ten" --season "2025-26"
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --conference "Big Ten" --season "2024-25" --csv
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --conferences all --season "2024-25" --csv
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --all-games --season "2024-25" --csv
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --all-games --seasons "2020-21,2021-22,2022-23,2023-24,2024-25" --csv
    python lambda/ncaab_fade_revenge_daily/tmp/analyze_ncaab_conference_rematch_su_ats.py --all-games --seasons "2020-21,...,2025-26" --csv --plot

Output: Stdout summary in 3 tables per run — regular_season, conf_tournament, ncaa_tournament
(conf_tournament = 10 days before NCAA start; bounds from config/season_dates.yaml ncaab).
Optional CSV to data/04_output/ with rematch_type and game_period columns.
"""
...
# Conference tournament proxy: N days before NCAA tournament start (both teams equally motivated)
CONF_TOURNEY_DAYS_BEFORE_NCAA = 10
...
def load_ncaab_period_bounds() -> dict:
    """
    Load NCAAB season bounds from config/season_dates.yaml.
    Returns dict: season -> {'conf_start': date, 'ncaa_start': date}.
    Bins: regular_season (game_date < conf_start), conf_tournament (conf_start <= game_date < ncaa_start), ncaa_tournament (game_date >= ncaa_start).
    """
    ...
    ncaab = config['ncaab']
    out = {}
    for season, dates in ncaab.items():
        ncaa_start_s = dates['tournament_start']
        ncaa_start = datetime.strptime(ncaa_start_s, '%Y-%m-%d').date()
        conf_start = ncaa_start - timedelta(days=CONF_TOURNEY_DAYS_BEFORE_NCAA)
        out[season] = {'conf_start': conf_start, 'ncaa_start': ncaa_start}
    return out
...
# -----------------------------------------------------------------------------
# Game period: regular_season | conf_tournament | ncaa_tournament
# -----------------------------------------------------------------------------
...
def add_game_period(
    ...
    """
    Add column game_period: 'regular_season', 'conf_tournament', or 'ncaa_tournament'.
    period_bounds[season] = {'conf_start': date, 'ncaa_start': date} (from load_ncaab_period_bounds).
    Uses row['season'] if present, else default_season. Leaves game_period None for unknown season.
    """
    df = df.copy()
    dates = pd.to_datetime(df['GAME_DATE']).dt.date
    season_col = df['season'] if 'season' in df.columns else default_season
    if isinstance(season_col, str):
        season_col = pd.Series([season_col] * len(df), index=df.index)

    result = pd.Series(index=df.index, dtype=object)
    for season in season_col.dropna().unique():
        mask = season_col == season
        if season not in period_bounds:
            result.loc[mask] = None
            continue
        conf_start = period_bounds[season]['conf_start']
        ncaa_start = period_bounds[season]['ncaa_start']
        d = dates.loc[mask]
        result.loc[mask] = np.where(
            d < conf_start,
            'regular_season',
            np.where(d < ncaa_start, 'conf_tournament', 'ncaa_tournament'),
        )
    df['game_period'] = result
    return df
...
def verify_focal_ats_cover(df: pd.DataFrame) -> int:
    ...
    if n_mismatch:
        ...
        LOG.info("ATS verification: all %s rematch rows match recomputed focal_ats_cover", len(rematch))
    return n_mismatch


def plot_fade_pnl(df: pd.DataFrame, out_path: Path) -> None:
    """
    Plot cumulative PnL by play number (no date gaps). Fade rematch (focal) team, flat 1u.
    X-axis = play number (1, 2, ...). Dotted vertical lines mark start of each season when present.
    Verifies focal_ats_cover against recomputed ATS before plotting; adds fade win rate to title.
    """
    n_mismatch = verify_focal_ats_cover(df)
    if n_mismatch:
        print(f"ATS verification: {n_mismatch} mismatches (see log). Plot uses stored focal_ats_cover.")

    rematch = df[df['rematch_type'].isin(('rematch 2nd', 'rematch 3rd'))].copy()
    rematch = rematch.dropna(subset=['focal_ats_cover'])
    if rematch.empty:
        LOG.warning("No rematch games with spread for PnL plot; skipping.")
        return
    rematch['GAME_DATE'] = pd.to_datetime(rematch['GAME_DATE'])
    rematch = rematch.sort_values('GAME_DATE').reset_index(drop=True)
    # Fade focal: we win when focal does NOT cover (focal_ats_cover is False)
    rematch['result'] = rematch['focal_ats_cover'].map(lambda x: -1 if x else 1)
    rematch['cum_pnl'] = rematch['result'].cumsum()

    n_wins = (rematch['result'] == 1).sum()
    n_losses = (rematch['result'] == -1).sum()
    fade_win_pct = 100.0 * n_wins / len(rematch) if len(rematch) else 0

    play_number = np.arange(1, len(rematch) + 1)
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(play_number, rematch['cum_pnl'].values, color='#2e7d32', linewidth=1.2)
    ax.axhline(0, color='gray', linestyle='--', linewidth=0.8)
    # Dotted verticals at season starts (when we have a season column)
    if 'season' in rematch.columns:
        season_changes = rematch['season'] != rematch['season'].shift(1)
        start_play_numbers = np.where(season_changes)[0] + 1  # 1-based play number
        for p in start_play_numbers:
            ax.axvline(p, color='gray', linestyle=':', linewidth=0.8)
    ax.set_xlabel('Play number')
    ax.set_ylabel('Cumulative PnL (flat 1u, fade rematch team)')
    ax.set_title(f'Backtest: fade rematch team ATS (n={len(rematch)} games) — fade win rate {fade_win_pct:.1f}% ({n_wins}W-{n_losses}L)')
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    LOG.info("Wrote PnL plot: %s (fade win rate %.1f%%)", out_path, fade_win_pct)
    print(f"PnL plot: {out_path} (fade win rate {fade_win_pct:.1f}%, {n_wins}W-{n_losses}L)")
...
def main():
    ...
    period_bounds = load_ncaab_period_bounds()
    ...
    if args.all_games:
        ...
        if len(seasons_list) == 1:
            ...
            df = add_game_period(df, period_bounds, one_season)
            # Replicate pre-split: one table for all games (no period filter)
            agg_all = aggregate_rematch_stats(df)
            print_summary(agg_all, "all games", one_season)
            # Then 3 tables by period
            for period in GAME_PERIODS:
                sub = df[df['game_period'] == period]
                if sub.empty:
                    continue
            ...
            if args.csv:
                ...
                print(f"CSV written: {out_path} (column: game_period)")
            if args.plot:
                out_dir.mkdir(parents=True, exist_ok=True)
                plot_fade_pnl(df, out_dir / f'ncaab_rematch_fade_pnl_{one_season}_{et_date}.png')
            _run_duckdb_queries(df, "all games")
            return

        combined = add_game_period(combined, period_bounds)
        # Replicate pre-split: one summary table for all games (no period filter)
        rows = []
        for season in seasons_list:
        ...
        for period in GAME_PERIODS:
            ...
            for season in seasons_list:
                ...
                rows.append(_all_games_summary_row(sub_s, season))
            rows.append(_all_games_summary_row(sub, 'all'))
            summary_df = pd.DataFrame(rows)
            print()
            print("=" * 80)
    ...
    if args.conferences is not None:
        ...
        combined = add_game_period(combined, period_bounds, args.season)
        # Replicate pre-split: one table for all conferences combined (no period filter)
        agg_all = aggregate_rematch_stats(combined)
        print_summary(agg_all, "(all conferences)", args.season)
        # Then 3 tables by period
        for period in GAME_PERIODS:
            sub = combined[combined['game_period'] == period]
            if sub.empty:
                continue
            agg = aggregate_rematch_stats(sub)
            print_summary(agg, f"(all conferences) — {period}", args.season)
        if args.csv:
            out_dir = PROJECT_ROOT / 'data' / '04_output'
            out_dir.mkdir(parents=True, exist_ok=True)
            ...
            LOG.info("Wrote CSV: %s", out_path)
            print(f"CSV written: {out_path} (columns: conference, rematch_type, game_period)")
    ...
    conf_df = add_pair_context(conf_df)
    conf_df = add_focal_and_results(conf_df)
    conf_df = add_game_period(conf_df, period_bounds, args.season)
    _log_rematch_home_away_flip(conf_df)
    _assert_rematch_dates_in_season(conf_df, season=args.season)

    # Replicate pre-split: one table for full conference (no period filter)
    agg_all = aggregate_rematch_stats(conf_df)
    print_summary(agg_all, args.conference, args.season)
    # Then 3 tables by period
    for period in GAME_PERIODS:
        sub = conf_df[conf_df['game_period'] == period]
        if sub.empty:
            continue
        agg = aggregate_rematch_stats(sub)
        print_summary(agg, f"{args.conference} — {period}", args.season)
    ...
    if args.csv:
        ...
        LOG.info("Wrote CSV: %s", out_path)
        print(f"CSV written: {out_path} (columns: rematch_type, game_period)")
