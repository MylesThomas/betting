...
- MLB Batter Hits pipeline built end-to-end (Steps 0–8 via /prop-research skill)
- Strategy: UNDER 0.5 hits, edge ≥ 2pp · OOS 4,600 bets · 53.5% hit · +426u · +9.27% ROI · net/MDD=4.15
- Two-stage model: OLS (→ yhat) + Logistic (yhat, offered_line → p_model). OLS r²=0.031 beats XGBoost r²=0.030 — linear market.
- Top feature: min_raw_implied_prob_under (r²=0.029, AUC=0.583). Market implied probs >> player rolling stats for this market.
- Models + spine uploaded to S3. Pipeline smoke-tested (5 plays 2026-07-27). Lambda built, NOT YET DEPLOYED.
- Session log: knowledge-base/raw/20260727-mlb-batter-hits.html

---

## 2026-07-24

- Added page: mlb-total-bases-dedup
- Source: 20260724-mlb-total-bases-dedup-analysis.html (generated by 20260724_grid_search_deduped.py)
- Covers: dedup effect on ROI (raw +7.5% → deduped +14.8% OOS at canonical strategy), book-count concentration split (2–4 books is IS money loser at −17.9%, 5+ books drives returns), live Lambda over-sizing implication (placing ~8 correlated bets per opportunity instead of 1)
