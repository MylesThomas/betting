"""
Test The Odds API Historical Data Availability

Purpose:
--------
Query The Odds API directly to check what historical data they have available
for different seasons. This hits the actual API, not our S3 storage.

Context:
--------
- Forward looking: Get current/upcoming events
- Backward looking: Use historical API to query past dates
- This helps understand The Odds API's data coverage before we try to fetch it

Usage:
------
python testing/test_odds_api_historical_availability.py --seasons 2021-22 2022-23
...
def check_historical_events(date, sport='basketball_nba'):
    ...
    Returns:
        dict: {'has_events': bool, 'num_events': int, 'error': str or None}
    """
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return {
                'has_events': len(data.get('data', [])) > 0,
...
def check_historical_props(date, event_id, sport='basketball_nba'):
    ...
    Returns:
        dict: {'has_props': bool, 'error': str or None}
    """
    timestamp = datetime.combine(date, datetime.min.time()).replace(hour=12).isoformat() + 'Z'
    ...
    try:
        ...
        if response.status_code == 200:
            return {'has_props': True, 'error': None}
        elif response.status_code == 422:
            return {'has_props': False, 'error': '422 - Props not available'}
        else:
            return {'has_props': False, 'error': f"HTTP {response.status_code}"}
    except Exception as e:
        return {'has_props': False, 'error': str(e)}
...
def analyze_season(season, num_samples=10):
    ...
    for i, date in enumerate(sample_dates, 1):
        ...
        if events_result['has_events']:
            ...
            # If we have events, try to check one event for props
            # (We'd need to actually fetch the event ID, but for now just note we have events)
            print()
        else:
    ...
    print(f"Game events available: {events_found}/{len(sample_dates)} dates ({events_pct:.1f}%)")
    ...
    if events_pct > 70:
        print(f"✅ The Odds API HAS GOOD historical coverage for {season}")
    elif events_pct > 30:
        print(f"⚠️  The Odds API HAS PARTIAL historical coverage for {season}")
    else:
        print(f"❌ The Odds API HAS POOR historical coverage for {season}")
...
def main():
    ...
    print(f"   Estimated API calls: {len(args.seasons) * args.samples}")
    print()
    ...
    print(f"\n💡 NOTE: This only checks for events, not player props specifically.")
    print(f"   Player props may have different availability than game lines.")
    print()
