"""
Build the joined player-sacks dataset for 2025 NFL REG season (v2).

v2 additions over v1:
  - prop_min_impl_over/under   — best available implied prob (most generous book)
  - prop_max_impl_over/under   — worst available implied prob (stingiest book)
  - prop_best_price_over/under — American odds from most generous book (lowest vig)
  - prop_book_spread_over/under — max_impl - min_impl (cross-book disagreement)
  These are used as model features in build_features.py to let models learn
  when market consensus is tight vs uncertain.
...
Output:
  ~/Downloads/tmp/nfl_sacks_joined_2025_v2.parquet
  ~/Downloads/tmp/nfl_sacks_joined_2025_v2.csv
...
Run:
  python src/nfl_sacks_modeling/scripts/build_sacks_dataset_v2.py
"""
...
OUT_PARQUET = OUT_DIR / "nfl_sacks_joined_2025_v2.parquet"
OUT_CSV     = OUT_DIR / "nfl_sacks_joined_2025_v2.csv"
...
def build_props() -> pd.DataFrame:
    ...
    def agg_player(g):
        ...
        # Best price = book with lowest implied prob (highest payout for bettor)
        ...
        return pd.Series({
            ...
            "prop_median_impl_over":   over_impl.median()  if len(over_impl)  else float("nan"),
            "prop_median_impl_under":  under_impl.median() if len(under_impl) else float("nan"),
            # v2: min/max implied prob and best price across books at 0.5 line
            ...
            # Cross-book disagreement (wide spread = books uncertain = potential model edge)
