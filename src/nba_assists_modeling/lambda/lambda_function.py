"""
NBA Assists Lambda orchestrator.
...
def lambda_handler(event, context):
    ...
    print(f"NBA Assists Lambda | mode={mode} | date={today_et}")

    scripts_dir   = root / "src" / "nba_assists_modeling" / "scripts"
    ...
    try:
        if mode == "spine_update":
            ...
            _publish_sns(topic_arn,
                subject=f"NBA assists spine updated — {today_et}",
            ...
            cmd = [sys.executable, str(scripts_dir / "settle_assists.py")]
            if gameday:
                cmd += ["--gameday", gameday]
        ...
        _publish_sns(topic_arn,
            subject=f"NBA assists Lambda FAILED — {mode} — {today_et}",
            message=f"Mode: {mode}\nDate: {today_et}\n\nError:\n{err_msg}")
        return {
            "statusCode": 500,
            "body": json.dumps({
