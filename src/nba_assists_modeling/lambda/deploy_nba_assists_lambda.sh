#!/bin/bash
###############################################################################
# Deploy NBA Assists Daily Lambda via container image (ECR)
#
# EventBridge schedule (all rules start DISABLED — enable before 2026-10-28):
#   Daily 11am ET → pipeline   (fetch props, score, email)
#   Daily 10am ET → settle     (settle prior-day games; no-op if none)
...
#   cd ~/dev/betting && bash src/nba_assists_modeling/lambda/deploy_nba_assists_lambda.sh
###############################################################################
...
ECR_REPO_NAME="nba-assists-daily"
IMAGE_TAG="${IMAGE_TAG:-latest}"
DOCKER_PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"

RULE_PIPELINE="nba-assists-pipeline-daily-11am-et"
RULE_SETTLE="nba-assists-settle-daily-10am-et"

CRON_DAILY_11AM="cron(0 16 * * ? *)"   # 11am ET (UTC-5 winter, UTC-4 summer; using 16 UTC)
CRON_DAILY_10AM="cron(0 15 * * ? *)"   # 10am ET
...
echo "================================================================================"
echo "DEPLOY NBA ASSISTS DAILY LAMBDA (CONTAINER)"
echo "================================================================================"
...
python -m py_compile \
  "src/nba_assists_modeling/lambda/lambda_function.py" \
  "src/nba_assists_modeling/scripts/update_spine.py" \
  "src/nba_assists_modeling/scripts/run_pipeline.py" \
  "src/nba_assists_modeling/scripts/settle_assists.py"
...
docker buildx build \
  ...
  --provenance=false \
  --load \
  -f "src/nba_assists_modeling/lambda/Dockerfile" \
...
for RULE_NAME in "$RULE_PIPELINE" "$RULE_SETTLE"; do
  if [ "$RULE_NAME" = "$RULE_PIPELINE" ]; then
    CRON="$CRON_DAILY_11AM"; MODE="pipeline"
  else
    CRON="$CRON_DAILY_10AM"; MODE="settle"
  fi

  aws events put-rule --name "$RULE_NAME" \
    --schedule-expression "$CRON" --state DISABLED \
    --description "NBA assists: $MODE" --region "$REGION" --output table
...
echo "  1. Upload spine:"
echo "       python src/nba_assists_modeling/scripts/update_spine.py"
...
echo "  3. Test pipeline:  aws lambda invoke --function-name $LAMBDA_NAME --payload '{\"mode\":\"pipeline\"}' /tmp/out.json"
echo "  4. Test settle:    aws lambda invoke --function-name $LAMBDA_NAME --payload '{\"mode\":\"settle\"}' /tmp/out.json"
echo ""
