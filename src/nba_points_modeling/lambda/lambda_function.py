"""
NBA Player Points Lambda orchestrator.

Modes (set via EventBridge payload {"mode": "..."}):
  spine_update  — Rebuild spine from S3 game logs, upload              (pre-season + weekly)
...
def lambda_handler(event, context):
    ...
    print(f"NBA Points Lambda | mode={mode} | date={today_et}")

    scripts_dir   = root / "src" / "nba_points_modeling" / "scripts"
    ...
    try:
        if mode == "spine_update":
            ...
            _publish_sns(topic_arn,
                subject=f"NBA points spine updated — {today_et}",
            ...
            cmd = [sys.executable, str(scripts_dir / "settle_points.py")]
            if gameday:
                cmd += ["--gameday", gameday]
        ...
        _publish_sns(topic_arn,
            subject=f"NBA points Lambda FAILED — {mode} — {today_et}",
            message=f"Mode: {mode}\nDate: {today_et}\n\nError:\n{err_msg}")
        return {
            "statusCode": 500,
            "body": json.dumps({
