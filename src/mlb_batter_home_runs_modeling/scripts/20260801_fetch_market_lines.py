"""
Backfill batter_home_runs Odds API lines for 2024-2026.
Idempotent — skips event IDs already saved in S3.

Uses ThreadPoolExecutor to fetch multiple events in parallel.

Output paths:
  S3:    s3://the-odds-api-mt/mlb/batter_home_runs_model/market_raw/{season}/{event_id}.parquet
  Local: ~/Downloads/tmp/mlb_batter_home_runs_market_raw.parquet  (merged all seasons)

Usage:
  python src/mlb_batter_home_runs_modeling/scripts/20260801_fetch_market_lines.py
  python src/mlb_batter_home_runs_modeling/scripts/20260801_fetch_market_lines.py --seasons 2026
  python src/mlb_batter_home_runs_modeling/scripts/20260801_fetch_market_lines.py --dry-run
  python src/mlb_batter_home_runs_modeling/scripts/20260801_fetch_market_lines.py --rebuild-local
"""
...
S3_PREFIX = "mlb/batter_home_runs_model/market_raw"
LOCAL_OUT = Path.home() / "Downloads/tmp/mlb_batter_home_runs_market_raw.parquet"
...
def rebuild_local_from_s3(seasons: list[int]) -> None:
    ...
    print(f"  Saved {len(combined):,} rows to {LOCAL_OUT}")
    print(f"  Columns: {combined.columns.tolist()}")
    print(f"  Seasons: {sorted(combined['season'].unique())}")
    print(f"  Books:   {sorted(combined['bookmaker'].unique())}")
...
def main() -> None:
    ...
    parser.add_argument("--rebuild-local", action="store_true",
                        help="Re-download all S3 parquets and build local merged file")
    args = parser.parse_args()

    if args.rebuild_local:
        rebuild_local_from_s3(args.seasons)
        return

    total = 0
    for season in args.seasons:
        total += process_season(season, dry_run=args.dry_run)

    print(f"\nDone. Total rows written: {total:,}")

    if not args.dry_run:
        print("\nRebuilding local merged file from S3...")
        rebuild_local_from_s3(args.seasons)
