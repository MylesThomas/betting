#!/bin/bash
###############################################################################
# Deploy MLB Batter Hits Lambda via container image (ECR)
#
# EventBridge schedule (all rules start DISABLED — enable after confirming):
#   Daily 8:30am ET → spine_update  (Statcast fetch + spine rebuild + email)
#   Daily 9:00am ET → combined      (settle yesterday + today's plays → ONE email)
#
# S3 assets are pre-uploaded by train_model.py:
...
#   cd ~/dev/betting && bash src/mlb_batter_hits_modeling/lambda/deploy_mlb_batter_hits_lambda.sh
###############################################################################
...
LAMBDA_NAME="mlb-batter-hits-daily"
ECR_REPO_NAME="mlb-batter-hits-daily"
IMAGE_TAG="${IMAGE_TAG:-latest}"
DOCKER_PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"

RULE_SPINE="mlb-hits-spine-daily-830am-et"
RULE_COMBINED="mlb-hits-combined-daily-9am-et"
...
echo "================================================================================"
echo "DEPLOY MLB BATTER HITS DAILY LAMBDA (CONTAINER)"
echo "================================================================================"
...
python -m py_compile \
  "src/mlb_batter_hits_modeling/lambda/lambda_function.py" \
  "src/mlb_batter_hits_modeling/scripts/update_spine.py" \
  "src/mlb_batter_hits_modeling/scripts/run_pipeline.py" \
  "src/mlb_batter_hits_modeling/scripts/settle_hits.py"
echo "✅ Syntax checks passed"

# ── S3 assets check ───────────────────────────────────────────────────────────
echo ""
echo "Step 2a: Checking S3 assets..."
aws s3 ls "s3://the-odds-api-mt/mlb/batter_hits_model/model/mlb_batter_hits_ols.joblib" \
  && echo "✅ OLS model present" \
  || { echo -e "${RED}❌ OLS model missing — run train_model.py first${NC}"; exit 1; }
aws s3 ls "s3://the-odds-api-mt/mlb/batter_hits_model/model/mlb_batter_hits_logit.joblib" \
  && echo "✅ Logistic model present" \
  || { echo -e "${RED}❌ Logistic model missing — run train_model.py first${NC}"; exit 1; }
aws s3 ls "s3://the-odds-api-mt/mlb/batter_hits_model/spine/mlb_batter_hits_spine.parquet" \
  && echo "✅ Spine present" \
  || { echo -e "${RED}❌ Spine missing — run train_model.py first${NC}"; exit 1; }
...
docker buildx build \
  ...
  --provenance=false \
  --load \
  -f "src/mlb_batter_hits_modeling/lambda/Dockerfile" \
...
for RULE_NAME in "$RULE_COMBINED" "$RULE_SPINE"; do
  ...
  aws events put-rule --name "$RULE_NAME" \
    ...
    --description "MLB batter hits: $MODE" --region "$REGION" --output table
...
echo "    --cli-binary-format raw-in-base64-out /tmp/hits_out.json && cat /tmp/hits_out.json"
echo ""
echo "Enable EventBridge rules when ready:"
