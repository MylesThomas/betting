#!/bin/bash
###############################################################################
# Deploy MLB Pitcher Outs Lambda via container image (ECR)
#
# EventBridge schedule (all rules start DISABLED — enable after confirming):
#   Daily 8:00am ET → spine_update  (rebuild spine from gamelogs weekly/daily)
#   Daily 8:30am ET → pipeline      (fetch live props, score, email bets)
#   Daily 8:30am ET → settle        (settle prior-day games, email P&L)
#
# S3 assets already in place:
#   s3://the-odds-api-mt/mlb/pitcher_outs_model/model/mlb_pitcher_outs_model.joblib
#   s3://the-odds-api-mt/mlb/pitcher_outs_model/model/mlb_pitcher_outs_residuals.npy
#   s3://the-odds-api-mt/mlb/pitcher_outs_model/model/mlb_pitcher_outs_encoders.joblib
#   s3://the-odds-api-mt/mlb/pitcher_outs_model/spine/mlb_pitcher_outs_spine.parquet
#
# Usage:
...
#   cd ~/dev/betting && bash src/mlb_pitcher_outs_modeling/lambda/deploy_mlb_pitcher_outs_lambda.sh
###############################################################################
...
LAMBDA_NAME="mlb-pitcher-outs-daily"
ECR_REPO_NAME="mlb-pitcher-outs-daily"
IMAGE_TAG="${IMAGE_TAG:-latest}"
DOCKER_PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"

RULE_SPINE="mlb-po-spine-daily-8am-et"
RULE_PIPELINE="mlb-po-pipeline-daily-830am-et"
RULE_SETTLE="mlb-po-settle-daily-830am-et"

CRON_SPINE="cron(0 12 * * ? *)"    # 8:00am ET daily (12:00 UTC, EDT=UTC-4)
CRON_PIPELINE="cron(30 12 * * ? *)" # 8:30am ET daily (12:30 UTC)
CRON_SETTLE="cron(30 12 * * ? *)"   # 8:30am ET daily (12:30 UTC)
...
echo "================================================================================"
echo "DEPLOY MLB PITCHER OUTS DAILY LAMBDA (CONTAINER)"
echo "================================================================================"
...
python -m py_compile \
  "src/mlb_pitcher_outs_modeling/lambda/lambda_function.py" \
  "src/mlb_pitcher_outs_modeling/scripts/build_spine.py" \
  "src/mlb_pitcher_outs_modeling/scripts/run_pipeline.py" \
  "src/mlb_pitcher_outs_modeling/scripts/settle_pitcher_outs.py"
echo "✅ Syntax checks passed"

# Verify S3 model artifacts exist
echo "Checking S3 model artifacts..."
aws s3 ls "s3://the-odds-api-mt/mlb/pitcher_outs_model/model/mlb_pitcher_outs_model.joblib" --region "$REGION" > /dev/null
aws s3 ls "s3://the-odds-api-mt/mlb/pitcher_outs_model/model/mlb_pitcher_outs_residuals.npy" --region "$REGION" > /dev/null
aws s3 ls "s3://the-odds-api-mt/mlb/pitcher_outs_model/spine/mlb_pitcher_outs_spine.parquet" --region "$REGION" > /dev/null
echo "✅ S3 artifacts confirmed"
...
docker buildx build \
  ...
  --provenance=false \
  --load \
  -f "src/mlb_pitcher_outs_modeling/lambda/Dockerfile" \
...
for RULE_NAME in "$RULE_PIPELINE" "$RULE_SETTLE" "$RULE_SPINE"; do
  ...
  aws events put-rule --name "$RULE_NAME" \
    ...
    --description "MLB pitcher outs: $MODE" --region "$REGION" --output table
...
echo "    --cli-binary-format raw-in-base64-out /tmp/po_out.json && cat /tmp/po_out.json"
echo ""
...
echo ""
echo "Pre-checklist before enabling:"
echo "  1. Verify pitcher gamelogs in S3: aws s3 ls s3://the-odds-api-mt/mlb/strikeouts_model/pitcher_gamelogs/"
echo "  2. Test pipeline mode manually (command above)"
echo "  3. Verify SES email arrives at mylescgthomas@gmail.com"
echo "  4. Run settle manually for a past gameday to verify settlement logic"
echo ""
