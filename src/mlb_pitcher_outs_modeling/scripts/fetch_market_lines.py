"""
Backfill pitcher_outs (+ alternate) Odds API lines for 2024-2026.
...
Output schema per row:
  ...
  Notes:
    - consensus_home/away_moneyline: median American odds across all books
      for that team's h2h side; used in spine to get pitcher's team moneyline.
    - home/away_run_line_point: almost always -1.5/+1.5; stored per row for
      completeness. Median across books.
    - home/away_run_line_odds: median run-line price across books.
    - In the spine, pitcher's team odds are resolved by joining to gamelogs
      on event_id and using is_home to pick home vs away columns.

Output paths:
  S3:    s3://the-odds-api-mt/mlb/pitcher_outs_model/market_raw/
         {season}/{event_id}.parquet
  Local: ~/Downloads/tmp/mlb_pitcher_outs_market_raw.parquet  (merged all seasons)

Usage:
  # Full backfill (all seasons):
  python src/mlb_pitcher_outs_modeling/scripts/fetch_market_lines.py

  # Single season:
  python src/mlb_pitcher_outs_modeling/scripts/fetch_market_lines.py --seasons 2026

  # Dry-run (print events that would be fetched, no API calls for odds):
  python src/mlb_pitcher_outs_modeling/scripts/fetch_market_lines.py --dry-run

  # Skip ahead to a specific date (useful for daily top-up after initial backfill):
  python src/mlb_pitcher_outs_modeling/scripts/fetch_market_lines.py --start-date 2026-07-01

  # Rebuild merged local parquet from S3 (no new API calls):
  python src/mlb_pitcher_outs_modeling/scripts/fetch_market_lines.py --rebuild-local
"""
...
SPORT         = "baseball_mlb"
# Fetch prop markets + game lines in one call so team odds are always paired with props.
PROP_MARKETS  = {"pitcher_outs", "pitcher_outs_alternate"}
GAME_MARKETS  = {"h2h", "spreads"}
MARKETS       = "pitcher_outs,pitcher_outs_alternate,h2h,spreads"
REGIONS       = "us,us2"
SLEEP_S       = 0.15
CREDIT_STOP   = 50_000

S3_BUCKET  = "the-odds-api-mt"
S3_PREFIX  = "mlb/pitcher_outs_model/market_raw"
LOCAL_OUT  = Path.home() / "Downloads/tmp/mlb_pitcher_outs_market_raw.parquet"

ET  = ZoneInfo("America/New_York")
UTC = ZoneInfo("UTC")

# Season date ranges — end is inclusive.
# 2026 end is today so the script stays current when run mid-season.
...
def snapshot_utc(commence_time_str: str) -> str:
    """Pre-game snapshot: 1 hour before the game's scheduled start time.

    Uses the event's commence_time so every game gets its own correct snapshot
    regardless of start time (day games, West Coast night games, etc.).
    """
...
def get_events_for_date(d: date) -> list[dict]:
    ...
    for attempt in range(3):
        try:
            ...
            # Deduplicate by event_id — historical API can return the same game twice
            seen: set[str] = set()
            unique = []
            for ev in events:
...
def fetch_event_odds(event_id: str, snapshot: str) -> tuple[list[dict], dict, int]:
    """Fetch all markets (props + game lines) for one event.

    Returns:
        prop_rows: list of dicts, one per (bookmaker, player, line) for pitcher_outs markets
    ...
    # Accumulators for game-level consensus odds (across books)
    ...
    for bm in bookmakers:
        ...
        for mkt in bm.get("markets", []):
            ...
            if mkt_key in PROP_MARKETS:
                # Player prop — one row per (book, player, line) pair
                ...
                # Game moneyline — collect prices keyed to home/away
                # Outcomes have team names as "name"; home_team / away_team resolved at caller
                # We store outcomes by position: first = away, second = home (Odds API convention)
                # But use name matching when possible.
                for o in outcomes:
                    if o.get("name") == data.get("home_team"):
                        home_h2h_prices.append(o["price"])
                    elif o.get("name") == data.get("away_team"):
                        away_h2h_prices.append(o["price"])

            elif mkt_key == "spreads":
                # Run line — collect points + prices keyed to home/away
                for o in outcomes:
                    if o.get("name") == data.get("home_team"):
    ...
    while d <= end:
        ...
        if events:
            ...
            if n_skip == len(events):
                ...
                print(f"  (skipped {skipped} already-fetched events)")
                skipped = 0
        ...
        for ev in events:
            ...
            # Use ET date so market game_date aligns with MLB stats API (which uses ET)
            # West Coast games starting after midnight UTC would otherwise land on the wrong day
            ...
            snapshot = snapshot_utc(ev.get("commence_time", ""))
            prop_rows, game_odds, remaining = fetch_event_odds(event_id, snapshot)
            time.sleep(SLEEP_S)
            ...
            if not prop_rows:
                # Write zero-row marker so this event is permanently skipped on future runs
            ...
            # Broadcast consensus game odds onto every prop row
...
def rebuild_local_from_s3(seasons: list[int]) -> None:
    """Download all per-event parquets from S3 and merge into local file."""
    s3c    = boto3.client("s3")
    frames = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch MLB pitcher_outs market lines from Odds API.")
