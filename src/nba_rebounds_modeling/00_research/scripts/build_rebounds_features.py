"""
Build rebounds model feature universe.

Outputs:
- rebounds_features.parquet: one row per player/date/game_id with rolling features + REB target
- rebounds_props.parquet: one row per bookmaker × posted line with odds, no-vig probs, REB outcome

Usage:
    python src/nba_rebounds_modeling/00_research/scripts/build_rebounds_features.py \
        --season "*" \
        --output ~/Downloads/tmp/rebounds_features.parquet \
        --output-props ~/Downloads/tmp/rebounds_props.parquet
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output",
        type=str,
        default="~/Downloads/tmp/rebounds_features.parquet",
    )
    p.add_argument(
        "--output-props",
        type=str,
        default="~/Downloads/tmp/rebounds_props.parquet",
    )
...
def load_logs(season: str, cache_dir: str, use_cache: bool, force: bool) -> pd.DataFrame:
    """Load player game logs from S3 with local cache. Includes FGA/FG3A/FTA and team_normalized."""
    ...
    if cached is not None and {"FGA", "FG3A", "FTA", "team_normalized"}.issubset(cached.columns):
    ...
    shot_cols = [c for c in ["FGA", "FG3A", "FTA"] if c in logs.columns]
    out = (
        logs[["season", "date", "GAME_ID", "player_normalized", "team_normalized", "MIN", "OREB", "DREB", "REB"] + shot_cols]
...
def load_v6_shot_profile(cache_dir: str) -> pd.DataFrame | None:
    """Load FGA/FG3A/FTA from v6 spread universe if available; returns None otherwise.
    When None, build_rolling_features falls back to shot columns already in logs."""
    v6_path = Path(cache_dir).expanduser() / "v6_spread_universe.parquet"
    if not v6_path.exists():
        print("v6_spread_universe.parquet not found — shot profile (FGA/FG3A/FTA) will be sourced from logs")
        return None
...
def load_game_spreads(season: str, cache_dir: str) -> pd.DataFrame:
    """Load team-level spread_signed from S3 historical game lines (Odds API)."""
    cache_path = Path(cache_dir).expanduser() / f"rebounds_nba_game_spreads_{season.replace(',', '_')}.parquet"
    cached = maybe_read_cache(cache_path, True, False)
    if cached is not None and {"season", "date", "team_normalized", "spread_signed"}.issubset(cached.columns):
        print(f"game_spreads: loaded from cache ({len(cached):,} rows)")
        return cached
    ...
    spread AS (
      ...
      FROM raw r WHERE {season_predicate('r', season)}
        AND home_line IS NOT NULL AND away_line IS NOT NULL
      GROUP BY season, date, home_team, away_team
    )
    SELECT season, date, home_team AS team_raw, home_spread AS spread_signed FROM spread
    UNION ALL
    SELECT season, date, away_team AS team_raw, away_spread AS spread_signed FROM spread
    """
    ...
    out = spread_df[["season", "date", "team_normalized", "spread_signed"]].drop_duplicates(
        subset=["season", "date", "team_normalized"]
    )
    maybe_write_cache(out, cache_path, True)
    print(f"game_spreads: fetched from S3 ({len(out):,} rows)")
    return out


def attach_spread(panel: pd.DataFrame, logs: pd.DataFrame, cache_dir: str, season: str) -> pd.DataFrame:
    """Attach spread_signed from v6 if available, else from S3 historical game lines."""
    try:
        v6_path = Path(cache_dir).expanduser() / "v6_spread_universe.parquet"
        ...
        print("spread_signed: attached from v6")
    except Exception:
        game_spreads = load_game_spreads(season, cache_dir)
        if "team_normalized" in logs.columns:
            player_team = logs[["season", "date", "player_normalized", "team_normalized"]].drop_duplicates(
                subset=["season", "date", "player_normalized"]
            )
            player_spread = player_team.merge(game_spreads, on=["season", "date", "team_normalized"], how="left")
            panel = panel.merge(
                player_spread[["season", "date", "player_normalized", "spread_signed"]],
                on=["season", "date", "player_normalized"],
                how="left",
            )
            filled = int(panel["spread_signed"].notna().sum())
            print(f"spread_signed: attached from S3 game lines ({filled:,} / {len(panel):,} filled)")
        else:
            panel["spread_signed"] = np.nan
            print("spread_signed: all NaN (no team info in logs)")
...
def build_rolling_features(logs: pd.DataFrame, v6_shots: pd.DataFrame | None) -> pd.DataFrame:
    """
    ...
    Shot columns (FGA/FG3A/FTA) come from v6 if available, else from logs directly.
    """
    base_cols = ["season", "date", "player_normalized", "game_id", "MIN", "OREB", "DREB", "REB"]
    shot_cols_in_logs = [c for c in ["FGA", "FG3A", "FTA"] if c in logs.columns]
    logs_ext = logs[base_cols + shot_cols_in_logs].copy()
    logs_ext["date"] = pd.to_datetime(logs_ext["date"]).dt.date.astype(str)

    if v6_shots is not None:
        ...
        # Prefer v6 columns when both exist
        for col in ["FGA", "FG3A", "FTA"]:
            v6_col = f"{col}_v6"
            if v6_col in logs_ext.columns:
                logs_ext[col] = logs_ext[v6_col].combine_first(logs_ext[col])
                logs_ext = logs_ext.drop(columns=[v6_col])
...
def main() -> None:
    ...
    output_v3_path = Path(args.output_props).expanduser()

    print(f"season={args.season}  cache_dir={cache_dir}  output={output_path}  props={output_v3_path}")

    # 1) Load source data
    logs  = load_logs(args.season, cache_dir, use_cache, force)
    props = load_props(args.season, cache_dir, use_cache, force)
    v6_shots = load_v6_shot_profile(cache_dir)  # None if v6 not available; falls back to logs

    # 2) Build market panel + per-book line table
    panel, book_line = build_market_panel(props, logs)
    panel = attach_spread(panel, logs, cache_dir, args.season)
