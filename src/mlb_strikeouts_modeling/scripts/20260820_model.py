"""
MLB Pitcher Strikeouts — v6 Model Training (umpire features)
=============================================================
Extends v5 by adding home plate umpire K-tendency features.
...
New in v6:
  ump_k_avg_career/c10/c20/season — rolling ump K tendency (prior games only)
  ump_k_delta/c10/c20/season      — ump tendency minus rolling league avg
...
Usage:
  uv run src/mlb_strikeouts_modeling/scripts/20260820_model.py
  uv run src/mlb_strikeouts_modeling/scripts/20260820_model.py --rebuild-spine
"""
...
ALL_FEATURES = [
    ...
    "consensus_line",
    # v5: consensus line odds bins (player-game level, book-independent)
    "over_price_bucket", "under_price_bucket",
    "over_price_bucket_fine", "under_price_bucket_fine",
    # v5: line and implied prob range (player-game level, book-independent)
    "min_line", "max_line",
    "min_over_prob", "max_over_prob",
    "min_under_prob", "max_under_prob",
    # v6: umpire K-tendency (game-level, no lookahead)
    ...
    # v6: ump identity as label-encoded categorical (XGBoost fixed effect per ump)
    "ump_name_encoded",
]
...
def load_spine() -> pd.DataFrame:
    """Load the labeled dataset (has all rolling + v5 features), join ump features,
    and deduplicate to one row per (player_key, game_date) for regression.
    """
    s3   = boto3.client("s3")
    ...
    # Bridge game_pk into labeled dataset via spine (labeled has no game_pk),
    # then join ump features on game_pk.
    ...
    lbl = lbl.merge(pk_map, on=["player_key", "game_date"], how="left")

    ump_body = s3.get_object(Bucket=S3_BUCKET, Key=UMP_KEY)["Body"].read()
    df_ump   = pd.read_parquet(BytesIO(ump_body))[["game_pk", "ump_name"] + UMP_COLS]
    lbl = lbl.merge(df_ump, on="game_pk", how="left")

    df = lbl.drop_duplicates(subset=["player_key", "game_date"], keep="first").copy()

    # Label-encode ump_name so XGBoost can learn a fixed effect per ump.
    # Unknown/null umps get their own label (filled before encoding).
    le = LabelEncoder()
    df["ump_name_encoded"] = le.fit_transform(df["ump_name"].fillna("__unknown__"))
...
def main():
    ...
    combos = [
        ...
        # ── v6: umpire combos ─────────────────────────────────────────────────
        # Simplest addition: delta on top of v4 baseline
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta"],
        # Best windowed delta variants
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta_c10"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta_c20"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta_season"],
        # Avg variants (not delta-adjusted)
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_avg_career"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_avg_c10"],
        # No market: does ump add signal even without consensus_line?
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home",
         "ump_k_delta"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home",
         "ump_k_delta_c10"],
        # Multiple ump features together
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta", "ump_k_delta_c10"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_k_delta", "ump_k_avg_season"],
        # Kitchen sink: best v5 combo + best ump feature
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "over_price_bucket", "under_price_bucket", "ump_k_delta"],
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "over_price_bucket", "under_price_bucket", "ump_k_delta_c10"],
        # ── v6: ump identity (categorical fixed effect) ───────────────────────
        # ump_name_encoded alone alongside v4 baseline
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_name_encoded"],
        # ump identity + best continuous delta
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "ump_name_encoded", "ump_k_delta"],
        # ump identity + v5 features
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "over_price_bucket", "under_price_bucket", "ump_name_encoded"],
        # ump identity + v5 + best delta
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home", "consensus_line",
         "over_price_bucket", "under_price_bucket", "ump_name_encoded", "ump_k_delta"],
        # no market: does ump identity add signal without consensus_line?
        ["k_roll_career", "k_roll_c5", "opp_k_against_season", "is_home",
         "ump_name_encoded"],
    ]
