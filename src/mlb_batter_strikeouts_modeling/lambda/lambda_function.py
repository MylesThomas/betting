"""
MLB Batter Strikeouts Lambda orchestrator.
...
Strategy: UNDER 1.5 · edge ≥ 5pp (raw)
OOF ROI: +8.94% (n=682, 2024 only — market coverage thin in 2025-2026)
...
VALID_MODES = {"pipeline", "settle", "combined"}
...
def _merge_html(html_pipeline: str, html_settle: str) -> str:
    ...
    def extract_style(html: str) -> str:
        import re as _re
        m = _re.search(r"<style[^>]*>(.*?)</style>", html, _re.DOTALL | _re.IGNORECASE)
...
def lambda_handler(event, context):
    ...
    print(f"MLB Batter Strikeouts Lambda | mode={mode} | date={today_et}")

    scripts_dir   = root / "src" / "mlb_batter_strikeouts_modeling" / "scripts"
    ...
    try:
        if mode == "pipeline":
            ...
            cmd = [sys.executable, str(scripts_dir / "settle_batter_strikeouts.py")]
            if gameday:
                cmd += ["--gameday", gameday]
            ...
            settle_out   = "/tmp/bs_settle_out.json"
            pipeline_out = "/tmp/bs_pipeline_out.json"

            settle_gameday = (event or {}).get("settle_gameday", None)
            settle_cmd = [sys.executable, str(scripts_dir / "settle_batter_strikeouts.py"),
                          "--output", settle_out]
            if settle_gameday:
            ...
            subject = (
                f"MLB Strikeouts — {n_plays} {play_word} today · "
                f"Yesterday: {yest_wins}W/{yest_loss}L {yest_units:+.2f}u · "
                f"Season: {s_wins}W/{s_loss}L {s_units:+.2f}u — {today_et}"
            )
