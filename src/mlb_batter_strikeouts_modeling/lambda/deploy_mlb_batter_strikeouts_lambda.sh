#!/bin/bash
###############################################################################
# Deploy MLB Batter Strikeouts Lambda via container image (ECR)
#
# EventBridge schedules (all rules start DISABLED — enable when ready):
#   Daily 9:00am ET → combined      (settle yesterday + today's plays → ONE email)
#
# S3 model artifact already in place:
#   s3://the-odds-api-mt/mlb/batter_strikeouts_model/artifacts/model_market_batter_lr.pkl
#
# Usage:
#   export ODDS_API_KEY="<key>"
#   export SES_SOURCE="<verified-ses-email>"
#   export SES_TO="mylescgthomas@gmail.com"
#   cd ~/dev/betting && bash src/mlb_batter_strikeouts_modeling/lambda/deploy_mlb_batter_strikeouts_lambda.sh
###############################################################################
...
LAMBDA_NAME="mlb-batter-strikeouts-daily"
ECR_REPO_NAME="mlb-batter-strikeouts-daily"
IMAGE_TAG="${IMAGE_TAG:-latest}"
DOCKER_PLATFORM="${DOCKER_PLATFORM:-linux/amd64}"

RULE_COMBINED="mlb-bs-combined-daily-9am-et"

CRON_COMBINED="cron(0 13 * * ? *)"  # 9:00am ET (13:00 UTC)
...
echo "================================================================================"
echo "DEPLOY MLB BATTER STRIKEOUTS DAILY LAMBDA (CONTAINER)"
echo "================================================================================"
...
python -m py_compile \
  "src/mlb_batter_strikeouts_modeling/lambda/lambda_function.py" \
  "src/mlb_batter_strikeouts_modeling/scripts/run_pipeline.py" \
  "src/mlb_batter_strikeouts_modeling/scripts/settle_batter_strikeouts.py"
...
docker buildx build \
  ...
  --provenance=false \
  --load \
  -f "src/mlb_batter_strikeouts_modeling/lambda/Dockerfile" \
...
ENV_VARS="ODDS_API_KEY=${ODDS_API_KEY},SES_SOURCE=${_SES_SOURCE},SES_TO=${_SES_TO}"
...
for RULE_NAME in "$RULE_COMBINED"; do
  ...
  aws events put-rule --name "$RULE_NAME" \
    ...
    --description "MLB batter strikeouts: $MODE" --region "$REGION" --output table
...
echo "    --cli-binary-format raw-in-base64-out /tmp/bs_out.json && cat /tmp/bs_out.json"
echo ""
echo "When ready to go live — enable EventBridge rule:"
