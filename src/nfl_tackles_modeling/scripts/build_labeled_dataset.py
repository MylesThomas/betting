"""
Build per-book labeled training dataset for NFL tackles model.

Old approach: one row per player-game with offered_line = median line across
  all books, consensus_over_prob = blended de-vigged probs mixing books that may
  offer different lines. Wrong when books disagree on the line.

New approach: one row per player-book-game. Each row scores that book's specific
  line against that book's own de-vigged P(under). Same player-game appears once
  per book that offered a complete two-sided market.
...
Key columns per row:
  ...
  consensus_under_price — this book's under price in American odds
  mkt_consensus_point   — median line across all books (context only)
  book               — bookmaker key

Wide context columns ({book}_point/over_price/under_price) are retained in every
  per-book row for analysis; they do not enter model training (BEST_FEATS is an
  explicit list).

Beat-rate features are computed once on the wide dataset (against the median
  consensus line) before expansion — they are player-level signals that do not
  vary per book.

Outputs:
  ~/Downloads/tmp/nfl_tackles_per_book.parquet   ← training / inference input
  ~/Downloads/tmp/nfl_tackles_labeled.parquet     ← old wide format, kept for reference

Run:
  python src/nfl_tackles_modeling/scripts/build_labeled_dataset.py
"""
...
OUT_PER_BOOK  = Path.home() / "Downloads" / "tmp" / "nfl_tackles_per_book.parquet"
OUT_WIDE      = Path.home() / "Downloads" / "tmp" / "nfl_tackles_labeled.parquet"
...
# ── Wide pivot (unchanged — intermediate join layer) ──────────────────────────

def pivot_to_wide(odds: pd.DataFrame, name_map: dict[str, str]) -> pd.DataFrame:
    """One row per (game_id, player_name_norm) with per-book point + prices."""
...
# ── Beat-rate features (computed on wide, player-level signal) ────────────────

def add_beat_rate_features(labeled: pd.DataFrame) -> pd.DataFrame:
    """Rolling over-rate per player against the median consensus line."""
    labeled = labeled.sort_values(["player_name_norm", "season", "week"]).reset_index(drop=True)
    labeled["_over_result"] = (labeled["tackles_combined"] > labeled["offered_line"]).astype(float)
...
def expand_to_per_book(wide: pd.DataFrame) -> pd.DataFrame:
    """
    Expand wide (one row per player-game) to per-book (one row per player-book-game).

    offered_line     → overwritten with this book's specific line
    market_under_prob   → de-vigged P(under) from this book only
    consensus_under_price → this book's under price (American)
    mkt_consensus_point → original median across all books (context)

    All wide {book}_point/over_price/under_price columns are retained for context.
    """
    # Rename median line to distinguish from the per-book line written below
    wide = wide.rename(columns={"offered_line": "mkt_consensus_point"})

    non_book_cols = [c for c in wide.columns
                     if not any(c.startswith(f"{b}_") for b in BOOKS)]
    ...
    for _, row in wide.iterrows():
        for book in BOOKS:
            ...
            new_row = {c: row[c] for c in wide.columns}   # all wide cols (incl. context)
            new_row["book"]                  = book
            new_row["offered_line"]       = float(pt)   # this book's line — model feature
...
def main():
    ...
    print(f"    Unique name match rate         : {labeled['player_name_norm'].nunique() / total_norm:.1%}")

    # Beat-rate features (computed on wide, against median offered_line)
    ...
    # SUMMARY_COLS references mkt_consensus_point but labeled still has offered_line here
    ...
    print(f"\n  Wide (reference) → {OUT_WIDE}  ({len(wide_out):,} rows, {len(wide_out.columns)} cols)")

    # ── Expand to per-book ─────────────────────────────────────────────────────
    print("\n  Expanding to per-book rows...")
    per_book = expand_to_per_book(labeled)

    # Column order: spine → book identifiers → model features → context → beat-rate → wide cols
    id_cols      = ["book", "offered_line",
    ...
    per_book_wide_cols = [c for c in per_book.columns
                          if any(c.startswith(f"{b}_") for b in BOOKS)]
    ...
    print(per_book["offered_line"].value_counts().sort_index().to_string())

    print(f"\n  Players with 2+ distinct lines across books (per gameday):")
