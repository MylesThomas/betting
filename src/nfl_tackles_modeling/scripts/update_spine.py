"""
Rebuild the NFL tackles spine for the current season and upload to S3.

Modes:
  --update  (default) Re-fetch current season PFR + snaps from nfl_data_py,
...
Run:
  python src/nfl_tackles_modeling/scripts/update_spine.py --season 2026
  python src/nfl_tackles_modeling/scripts/update_spine.py --season 2026 --verify
"""
...
from build_historical_spine import (
    ...
    build_game_context, build_id_bridge, build_opp_features,
    build_opponent_map, build_pfr_tackles, build_snap_counts,
)

S3_BUCKET          = "the-odds-api-mt"
S3_KEY             = "nfl/tackles_model/spine/nfl_tackles_historical_spine.parquet"
...
def build_spine_for_seasons(seasons: list[int]) -> pd.DataFrame:
    ...
    joined_id = has_id.merge(
        ...
        on=["game_id", "season", "team", "player_id"], how="left",
    )
    joined_name = no_id.merge(
        snap_cols.rename(columns={"player_name_snap": "player_name"}),
        on=["game_id", "season", "team", "player_name"], how="left",
    )
...
def compare_spines(ref: pd.DataFrame, fresh: pd.DataFrame) -> bool:
    ...
    if removed:
        ...
        return False
    if added:
        print(f"  INFO: {len(added)} new game_ids (expected — new completed games)")
    numeric_cols = [c for c in ["tackles_combined", "defense_snaps", "defense_pct"]
                    if c in ref.columns and c in fresh.columns]
    common_ids = set(ref["game_id"]) & set(fresh["game_id"])
    ref_c   = ref[ref["game_id"].isin(common_ids)].sort_values(["game_id", "pfr_player_id"]).reset_index(drop=True)
    fresh_c = fresh[fresh["game_id"].isin(common_ids)].sort_values(["game_id", "pfr_player_id"]).reset_index(drop=True)
    n_diffs = 0
...
def main():
    ...
    print(f"\nNFL Tackles Spine — mode={mode}  season={season}")
    print("=" * 55)
    ...
    if historical.empty:
        ...
        combined    = combined.sort_values(["pfr_player_id", "season", "week"]).reset_index(drop=True)
