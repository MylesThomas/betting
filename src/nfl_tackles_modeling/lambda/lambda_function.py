"""
NFL Tackles Lambda orchestrator.
...
Env vars:
  ...
  SETTLEMENT_SES_SOURCE  (optional; verified SES sender for HTML emails)
  SETTLEMENT_SES_TO      (optional; comma-separated recipients)
...
def lambda_handler(event, context):
    ...
    print(f"NFL Tackles Lambda | mode={mode} | date={today_et} | season={nfl_season}")

    scripts_dir = root / "src" / "nfl_tackles_modeling" / "scripts"
    ...
    try:
        if mode == "spine_update":
            ...
            _publish_sns(
                topic_arn,
                subject=f"NFL tackles spine updated — {today_et}",
            ...
            _publish_sns(
                topic_arn,
                subject=f"NFL tackles spine verified — {today_et}",
            ...
            out = _run_capture(
                [sys.executable, str(scripts_dir / "settle_tackles.py")],
                cwd=root,
            )
            step_results.append({"step": "settle", "status": "ok"})
            # settle_tackles.py sends its own SES + SNS settlement email
        ...
        _publish_sns(
            topic_arn,
            subject=f"NFL tackles Lambda FAILED — {mode} — {today_et}",
            message=f"Mode: {mode}\nDate: {today_et}\nSeason: {nfl_season}\n\nError:\n{err_msg}",
        )
        return {
            "statusCode": 500,
