"""
NFL Rush Attempts Lambda orchestrator.
...
def lambda_handler(event, context):
    ...
    print(f"NFL Rush Attempts Lambda | mode={mode} | date={today_et} | season={nfl_season}")

    scripts_dir  = root / "src" / "nfl_rush_attempts_modeling" / "scripts"
    ...
    try:
        if mode == "spine_update":
            ...
            _publish_sns(
                topic_arn,
                subject=f"NFL rush attempts spine updated — {today_et}",
            ...
            _publish_sns(
                topic_arn,
                subject=f"NFL rush attempts spine verified — {today_et}",
            ...
            out = _run_capture(
                [sys.executable, str(scripts_dir / "settle_rush_attempts.py")],
                cwd=root,
            )
        ...
        _publish_sns(
            topic_arn,
            subject=f"NFL rush attempts Lambda FAILED — {mode} — {today_et}",
            message=f"Mode: {mode}\nDate: {today_et}\nSeason: {nfl_season}\n\nError:\n{err_msg}",
        )
        return {
            "statusCode": 500,
